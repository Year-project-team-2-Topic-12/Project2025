...
> 🧭 [README](../README.md) · **Чекпойнт 4 — Сервис** · ← [CP3 · Классические ML](../ML.ipynb) · [CP5 · ML-эксперименты](../ML_Experiments.ipynb) →
...
python >= 3.13
```
* Используйте скрипт автоматизации для установки всех зависимостей (включая библиотеки для безопасности). Навигация по меню — стрелки `↑`/`↓` или `j`/`k`, запуск пункта — `Enter`/`Space`:
```text
./bootstrap.sh
# Выберите пункт 🧰 Install ALL deps (python + frontend)
```

* Примените миграции Alembic (создаст файл базы данных):
```text
./bootstrap.sh
# Выберите пункт 🆙 Alembic upgrade head
```
